...
from agent.scanners import get_findings_store
...
# The agent (system prompt, sampling, tool registration) is built by engine.py —
# the same construction the FastAPI backend uses, so the two frontends never drift.
print(f"[cyberhackmythos] {get_sandbox().describe()}")
agent = build_agent(enable_mcp=True)
